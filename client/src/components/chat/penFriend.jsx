...
export default function PenFriend({friend}) {
    ...
    return (
      <li onClick={handleClick} className={'bg-mainColor pl-6 py-4 pr-4 grid grid-rows-2 grid-cols-9 items-center justify-center cursor-pointer border-t-2 transition-all' + (activePartner?.id === friend.id ? ' shadow-activeFriend' : '')}>
          ...
          <span className={'row-start-2 row-span-1 col-start-3 col-span-7 indent-2 ' + ((notReadCnt && !myMessage) ? ' text-title font-semibold col-span-5': '')}>{(user.id === lastMessage.sender ? 'You: ' : '') + lastMessage.content}</span>
          {notReadCnt > 0 && !myMessage && <span className={'w-6 h-6 row-start-2 row-span-1 col-start-8 col-span-2 justify-self-end rounded-full bg-title text-mainColor flex justify-center items-center'}>{notReadCnt}</span>}
